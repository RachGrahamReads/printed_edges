...
export default function CreatePage() {
  ...
  // Cover image upload (optional, separate from PDF)
  const [coverImageFile, setCoverImageFile] = useState<File | null>(null);
  const [coverImageUrl, setCoverImageUrl] = useState<string | null>(null);

  // Manual trim size and page count (can be overridden by PDF)
  const [manualTrimWidth, setManualTrimWidth] = useState<number>(6);
  const [manualTrimHeight, setManualTrimHeight] = useState<number>(9);
  const [manualPageCount, setManualPageCount] = useState<number>(200);
  const [useManualSettings, setUseManualSettings] = useState<boolean>(true);
  ...
  // Auto-show preview when edge images are ready
  useEffect(() => {
    const hasEdgeDesign = sideEdgeImage || topEdgeImage || bottomEdgeImage || topEdgeColor !== "none" || bottomEdgeColor !== "none";
    if (hasEdgeDesign && !isLoadingPreview) {
      setShowPreview(true);
      // Auto-generate mockup when preview is ready
      if (sideEdgeImage) {
        setTimeout(() => {
          generateMockup();
        }, 100);
  ...
  const handlePdfUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === 'application/pdf') {
      ...
      setUseManualSettings(false); // PDF overrides manual settings
  ...
  // Handle cover image upload (for mockup)
  const handleCoverImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      reader.onload = (e) => {
        ...
        setMockupUrl(null); // Reset mockup when cover changes
      };
  ...
  const handlePreview = async () => {
    // Can generate mockup with just edge image (no PDF required)
    const hasEdgeDesign = sideEdgeImage || topEdgeImage || bottomEdgeImage || topEdgeColor !== "none" || bottomEdgeColor !== "none";

    if (!hasEdgeDesign) {
      setPreviewError('Please upload at least one edge design to generate a preview');
      return;
    }
    ...
    try {
      // If PDF exists, load it for full preview
      ...
      // Check if PDF preview is ready OR if we can just generate mockup
      if (pdfFile && isPreviewReady()) {
        // Full preview with PDF
        setShowPreview(true);
        setViewMode("edge");
        ...
        // Mockup-only mode (no PDF)
        setShowPreview(true);
        setViewMode("edge");
      }

      // Auto-generate mockup when preview is shown
      generateMockup();
    } catch (error) {
  ...
  // Generate mockup from cover image (from PDF or uploaded) and edge design
  const generateMockup = async () => {
    // Need at least side edge image to generate mockup
    if (!sideEdgeImage) {
      return;
    }
    ...
    try {
      ...
      // Priority 1: Use uploaded cover image if available
      if (coverImageUrl) {
        coverBase64 = coverImageUrl.split(',')[1];
        coverDataUrl = coverImageUrl;
      }
      // Priority 2: Extract first page from PDF if available
      else if (pdfDocument) {
        const firstPage = await pdfDocument.getPage(1);
        const viewport = firstPage.getViewport({ scale: 2 }); // High resolution
        ...
        coverBase64 = coverDataUrl.split(',')[1];
      }
      // If no cover image at all, mockup will show template only

      // Convert edge image to base64
      let edgeBase64: string | undefined;
      if (sideEdgeImage) {
        edgeBase64 = sideEdgeImage.split(',')[1];
      }
      ...
      // Call Edge Function to generate mockup
      const { data, error: functionError } = await supabase.functions.invoke('generate-book-mockup', {
        body: {
          coverImageBase64: coverBase64,
          edgeDesignBase64: edgeBase64,
          trimWidth: trimWidthToUse,
          trimHeight: trimHeightToUse,
          pageCount: pageCountToUse,
          scaleMode: scaleMode // Pass the scale mode to respect the preview settings
        }
      });
      ...
      if (data?.success && data?.mockupUrl) {
        ...
        if (coverDataUrl) {
          setCoverImagePreview(coverDataUrl);
        }
      } else {
  ...
  const handleProcessPdf = async () => {
    ...
    if (!pdfFile) {
      const confirmUpload = window.confirm(
        'PDF Required: You need to upload a PDF file to process it with edge designs.\n\n' +
        'Would you like to upload a PDF now?\n\n' +
        'Note: You can generate mockups without a PDF, but processing requires a PDF file.'
      );
      if (confirmUpload) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="max-w-6xl mx-auto p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold mb-4">Preview & Process Your Edge Images</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Design your custom book edges in 3 simple stages
            {!user && " • Sign in to process and download your files"}
          </p>
        </div>

        <div className="grid lg:grid-cols-3 gap-8 max-w-[2000px] mx-auto">
          {/* Left Column - Stages 1 & 2 (1/3 width) */}
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card className="overflow-hidden">
              <div className="bg-blue-600 text-white px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold">Book Size</h2>
                </div>
              </div>
              <CardContent className="space-y-4 pt-6">
                {/* PDF Upload Section */}
                <div className="space-y-3">
                  <Label className="font-semibold text-base">Upload PDF</Label>
                  <Input
                    id="pdf-upload"
                    type="file"
                  ...
                  {pdfFile && (
                    <div className="space-y-2">
                      ...
                      {/* PDF has bleed checkbox */}
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <div className="flex items-center space-x-3">
                          <input
                            id="has-bleed"
                            type="checkbox"
                            checked={bleedType === "has_bleed"}
                            onChange={(e) => setBleedType(e.target.checked ? "has_bleed" : "add_bleed")}
                ...
                <div className="space-y-3">
                  <Label className="font-semibold text-base">Input Custom Dimensions</Label>
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <Label htmlFor="manual-width" className="text-xs text-gray-600">Width (inches)</Label>
                      <Input
                        id="manual-width"
                        type="number"
                        min="4"
                        ...
                        onChange={(e) => {
                          ...
                          setManualTrimWidth(isNaN(value) ? 0 : value);
                        }}
                        disabled={!useManualSettings}
                        className="mt-1"
                      />
                    </div>
                    <div>
                      <Label htmlFor="manual-height" className="text-xs text-gray-600">Height (inches)</Label>
                      <Input
                        id="manual-height"
                        type="number"
                        min="6"
                        ...
                        onChange={(e) => {
                          const value = e.target.value === '' ? 0 : parseFloat(e.target.value);
                          setManualTrimHeight(isNaN(value) ? 0 : value);
                        }}
                        disabled={!useManualSettings}
                        className="mt-1"
                      />
                    ...
                    <div>
                      <Label htmlFor="manual-pages" className="text-xs text-gray-600">Total Pages</Label>
                      <Input
                        id="manual-pages"
                        type="number"
                        min="20"
                        ...
                        onChange={(e) => {
                          ...
                          setManualPageCount(isNaN(value) ? 0 : value);
                        }}
                        disabled={!useManualSettings}
                        className="mt-1"
                      />
                  ...
                  {!useManualSettings && (
                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                      <p className="text-xs text-blue-700">
                        <strong>Using PDF dimensions:</strong> {bookWidth}"×{bookHeight}", {totalPages} pages
                      </p>
                    </div>
                  )}
                </div>
            ...
            <Card className="overflow-hidden">
              <div className="bg-purple-600 text-white px-6 py-4">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-white text-purple-600 rounded-full flex items-center justify-center text-sm font-bold">2</div>
                  <h2 className="text-xl font-bold">Edge Design</h2>
                </div>
              </div>
              <CardContent className="space-y-4 pt-6">
                {/* Template Download */}
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                  <div className="flex flex-row items-center justify-between lg:flex-col lg:items-start gap-2">
                    <p className="text-sm font-medium text-blue-800 lg:mb-0">Recommended Image Size: {numLeaves} x {((bleedType === "add_bleed" ? (useManualSettings ? manualTrimHeight : bookHeight) + 0.25 : (useManualSettings ? manualTrimHeight : bookHeight)) * 285.7).toFixed(0)}px</p>
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={generateTemplate}
                      className="text-xs shrink-0 lg:w-full"
                    >
                      📄 Download Template
                    </Button>
                  </div>
                ...
                {/* Side Edge Image */}
                <div>
                  <Label htmlFor="sideImage" className="text-sm font-semibold mb-2 block">Upload Edge Design</Label>
                  <Input
                    id="sideImage"
                    type="file"
                      accept="image/*"
                  ...
                  <div className="bg-white p-3 rounded-lg border border-gray-200">
                    <Label className="text-xs font-medium mb-2 block">Edge Colors</Label>
                    <div className="grid grid-cols-2 gap-2">
                      {/* Top Edge Color */}
                      <div>
                        <Label htmlFor="topEdgeColor" className="text-xs text-gray-500 block mb-1">Top</Label>
                        <div className="flex gap-1 items-center">
                          <select
                            id="topEdgeColor"
                            value={topEdgeColor}
                            onChange={(e) => setTopEdgeColor(e.target.value)}
                            className="px-2 py-1.5 text-xs border border-gray-300 rounded-md flex-1"
                      ...
                      {/* Bottom Edge Color */}
                      <div>
                        <Label htmlFor="bottomEdgeColor" className="text-xs text-gray-500 block mb-1">Bottom</Label>
                        <div className="flex gap-1 items-center">
                          <select
                            id="bottomEdgeColor"
            ...
            {(isProcessing || processedPdfUrl || processingError) && (
            <Card className="overflow-hidden">
              <CardContent className="space-y-4 pt-6">
          ...
          {/* Right Column - Stage 3 Preview (2/3 width) */}
          <div className="lg:col-span-2">
          <Card className="overflow-hidden h-full">
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-4">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-white text-purple-600 rounded-full flex items-center justify-center text-sm font-bold">3</div>
                <h2 className="text-xl font-bold">Preview & Process</h2>
              </div>
            </div>
            <CardContent className="p-6">
              ...
              {!showPreview ? (
                ...
                <div className="space-y-4">
                  ...
                  {/* 3D Mockup and Edge Preview - Side by Side */}
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 bg-gradient-to-b from-gray-50 to-gray-100 p-6 rounded-lg">
                      <div className="flex flex-col items-center space-y-6">
                        ...
                        <div className="w-full max-w-md">
                          <Label htmlFor="cover-upload-mockup" className="text-sm font-semibold mb-2 block">Add Cover Image (Optional)</Label>
                          <p className="text-xs text-gray-500 mb-2">
                            Upload a cover image for mockup preview. If not provided, uses PDF first page.
                          </p>
                          <Input
                            id="cover-upload-mockup"
                            type="file"
                            accept="image/*"
                            onChange={handleCoverImageUpload}
                            className="border-dashed border-2 h-10 hover:border-green-300 transition-colors"
                          />
                          {coverImageUrl && (
                            <div className="flex items-center gap-2 text-sm mt-2">
                              <CheckCircle className="h-4 w-4 text-green-600" />
                              <span className="text-green-600">Cover image uploaded</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex flex-col items-center space-y-4 max-w-4xl w-full">
                          {isGeneratingMockup ? (
                            <div className="bg-white rounded-lg p-12 flex items-center justify-center min-h-[400px] w-full">
                              <div className="text-center">
                                <Loader2 className="h-12 w-12 animate-spin text-blue-600 mx-auto mb-4" />
                                <p className="text-gray-600">Generating your 3D mockup...</p>
                              </div>
                            </div>
                          ) : mockupError ? (
                            <div className="bg-red-50 border border-red-200 rounded-lg p-6 w-full max-w-md">
                              <div className="flex items-start gap-2">
                                ...
                                <div>
                                  <p className="text-sm font-medium text-red-800">Mockup Generation Error</p>
                                  <p className="text-xs text-red-700 mt-1">{mockupError}</p>
                                </div>
                              </div>
                            </div>
                          ) : mockupUrl ? (
                            ...
                            <div className="bg-white rounded-lg p-12 flex items-center justify-center min-h-[400px] w-full border-2 border-dashed border-gray-300">
                              <div className="text-center">
                                <p className="text-gray-500 mb-4">Mockup will appear here once generated</p>
                                {sideEdgeImage && (
                                  <Button
                                    onClick={generateMockup}
                                    variant="outline"
                    ...
                    {/* Edge Preview - Compact Sidebar on Right */}
                    {sideEdgeImage && (
                      <div className="w-64 flex-shrink-0">
                        <div className="bg-white border border-gray-200 rounded-lg p-4 sticky top-4">
                          <div className="mb-3">
                            <h4 className="text-sm font-semibold text-gray-700">Edge Design Preview</h4>
                          </div>

                          <div className="space-y-3">
                            {/* Info Section - Above canvas */}
                            <div className="space-y-2">
                              {/* Color Edges Info */}
                              {(topEdgeColor !== "none" || bottomEdgeColor !== "none") && (
                                <div className="text-xs text-gray-600 flex items-center flex-wrap gap-x-3 gap-y-1">
                                  {topEdgeColor !== "none" && (
                              ...
                              <div className="relative">
                                <div className="flex items-center justify-between">
                                  <Label htmlFor="scaleMode-preview" className="text-xs font-medium text-gray-700">Image Scaling</Label>
                                  <button
                                    type="button"
                                ...
                                {showScaleModeInfo && (
                                  <div
                                    ...
                                    className="absolute top-full left-0 mt-2 p-3 bg-white border border-gray-300 rounded-lg shadow-lg z-50 w-72"
                                  >
                                    <div className="space-y-2">
                                      <div className="border-b pb-2">
                                        <h4 className="font-semibold text-xs">Edge Image Scaling Modes</h4>
                                      </div>
                                      <div>
                                        <p className="font-medium text-xs text-green-700">Fill (Recommended)</p>
                                        <p className="text-xs text-gray-600">Scales to fit perfectly. May crop parts of the image.</p>
                                      </div>
                                      <div>
                                        <p className="font-medium text-xs text-purple-700">Fit</p>
                                        <p className="text-xs text-gray-600">Shows entire image, may have gaps.</p>
                                      </div>
                                      <div>
                                        <p className="font-medium text-xs text-gray-700">None</p>
                                        <p className="text-xs text-gray-600">Uses original size - best if sized correctly.</p>
                                      </div>
                                      <div>
                                        <p className="font-medium text-xs text-blue-700">Stretch</p>
                                        <p className="text-xs text-gray-600">May distort - good for abstracts.</p>
                                      </div>
                                      <div>
                                        <p className="font-medium text-xs text-orange-700">Extend Sides</p>
                                        <p className="text-xs text-gray-600">Centers and extends edges to fill gaps.</p>
                                      </div>
                                      <button
                            ...
                            <div className="flex-shrink-0">
                              <div className="relative bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-3">
                                <canvas
                                  ref={sideEdgeCanvasRef}
                                  className="rounded shadow-md border border-gray-300 bg-white mx-auto"
                                  style={{
                                    height: '250px',
                                ...
                                />
                                {!canvasReady && (
                                  <div className="absolute inset-3 bg-gray-100 rounded border border-gray-200 flex items-center justify-center">
                                    <div className="text-center">
                            ...
                            {/* Bleed zones with inline button */}
                            {showBleedZones && (
                              <div className="flex items-center justify-between text-xs text-gray-500">
                                <p>
                                  <span className="text-red-600">■</span> Bleed zones •
                                  <span className="text-blue-600"> ■</span> Buffer zones
                                </p>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => setShowBleedZones(false)}
                            ...
                            {!showBleedZones && (
                              <div className="flex items-center justify-between text-xs text-gray-500">
                                <p>Bleed/buffer zones hidden</p>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => setShowBleedZones(true)}
                                  className="h-6 text-xs px-2"
                                >
                                  <Eye className="h-3 w-3 mr-1" />
                                  Show
              ...
              {/* Process PDF Button - Full Width at Bottom */}
              <div className="mt-6 pt-6 border-t border-gray-200">
                <Button
                  onClick={handleProcessPdf}
                  disabled={isProcessing || !showPreview || !pdfFile}
                  className="w-full"
                  size="lg"
                >
                  {!pdfFile ? (
