-- Update auto-profile creation trigger to populate first_name and last_name columns
...
BEGIN
  ...
  INSERT INTO public.users (id, email, first_name, last_name, name, created_at, is_admin)
  VALUES (
    NEW.id,
    NEW.email,
    NULLIF(first_name, ''), -- Use NULL if empty string
    NULLIF(last_name, ''),  -- Use NULL if empty string
...
COMMENT ON FUNCTION public.handle_new_user() IS 'Automatically creates user profile with separate first_name and last_name fields when a new user signs up through Supabase Auth';