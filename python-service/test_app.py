...
def process_files():
    try:
        ...
        return jsonify({
            "status": "success",
            "message": "Files received successfully! PDF processing is temporarily disabled for testing.",
            "received_files": {
...
def health():
    return jsonify({"status": "healthy", "service": "test_pdf_processor"})

@app.route("/", methods=["GET"])
def root():
    return jsonify({
        "status": "Test Python service is running", 
        "port": os.environ.get("PORT", "Not set"),
        "service": "test_pdf_service"
    })

if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5001))
    print(f"Test PDF service starting on port {port}")
