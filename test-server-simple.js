...
async function processPDF(requestData) {
  ...
  // Helper function to slice side edge images
  async function sliceSideEdgeImage(base64Data, leafNumber, totalLeaves, stripWidth, stripHeight, flipHorizontally = false) {
    try {
      // Create image from base64
      const imageBuffer = Buffer.from(base64Data, 'base64');
      const img = await loadImage(imageBuffer);

      // Always take exactly 1 pixel wide slice, positioned across the image width
      const sliceWidth = 1;
      const sliceStartX = Math.floor((leafNumber / totalLeaves) * img.width);

      console.log(`Slicing side edge: leaf ${leafNumber + 1}/${totalLeaves}, image size=${img.width}x${img.height}, taking 1px slice from x=${sliceStartX}`);

      // Create canvas for the slice
      const canvas = createCanvas(stripWidth, stripHeight);
      const ctx = canvas.getContext('2d');

      // Disable image smoothing to prevent fading/interpolation
      ...
      // Draw the slice, stretched to fit the strip dimensions
      ctx.drawImage(img, sliceStartX, 0, sliceWidth, img.height, 0, 0, stripWidth, stripHeight);

      return canvas.toBuffer('image/png');
    } catch (error) {
      console.error('Error slicing side edge image:', error);
      return null;
    }
  }

  // Helper function to slice top/bottom edge images (horizontal slicing)
  async function sliceTopBottomEdgeImage(base64Data, leafNumber, totalLeaves, stripWidth, stripHeight, flipHorizontally = false) {
    try {
      // Create image from base64
      const imageBuffer = Buffer.from(base64Data, 'base64');
      const img = await loadImage(imageBuffer);

      // Always take exactly 1 pixel tall slice, positioned across the image height
      const sliceHeight = 1;
      const sliceStartY = Math.floor((leafNumber / totalLeaves) * img.height);

      console.log(`Slicing top/bottom edge: leaf ${leafNumber + 1}/${totalLeaves}, image size=${img.width}x${img.height}, taking 1px slice from y=${sliceStartY}`);
      ...
      // Apply flip if needed (horizontal flip for even pages)
      if (flipHorizontally) {
        ctx.scale(-1, 1);
        ctx.translate(-stripWidth, 0);
      }
      ...
      // Draw the slice, stretched to fit the strip dimensions
      ctx.drawImage(img, 0, sliceStartY, img.width, sliceHeight, 0, 0, stripWidth, stripHeight);

      return canvas.toBuffer('image/png');
    } catch (error) {
      console.error('Error slicing top/bottom edge image:', error);
      return null;
    }
  }
  ...
  for (let pageNum = 0; pageNum < pages.length; pageNum++) { // Process all pages
    ...
    // Process side edges with slicing
    if (requestData.edgeType === 'side-only' || requestData.edgeImages.side) {
      try {
        ...
        // Create slice if not cached
        if (!sideImageCache[cacheKey]) {
          const slicedImageBuffer = await sliceSideEdgeImage(
            base64Data,
            leafNumber,
            numLeaves,
        ...
        if (sideImageCache[cacheKey]) {
          ...
          console.log(`Added sliced side edge image to page ${pageNum + 1} (${flipHorizontally ? 'flipped' : 'normal'})`);
        }
      } catch (error) {
      ...
      if (requestData.edgeImages.top) {
        try {
          ...
          if (!topImageCache[cacheKey]) {
            const slicedImageBuffer = await sliceTopBottomEdgeImage(
              base64Data,
              leafNumber,
              numLeaves,
              Math.round(newWidth),
          ...
          if (topImageCache[cacheKey]) {
            ...
            console.log(`Added sliced top edge image to page ${pageNum + 1} (${flipHorizontally ? 'flipped' : 'normal'})`);
          }
        } catch (error) {
      ...
      if (requestData.edgeImages.bottom) {
        try {
          ...
          if (bottomImageCache[cacheKey]) {
            ...
            console.log(`Added sliced bottom edge image to page ${pageNum + 1} (${flipHorizontally ? 'flipped' : 'normal'})`);
          }
        } catch (error) {
